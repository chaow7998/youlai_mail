<template>
   <view class="container" v-if="hasLogin"> <!-- memberId -->
    ...
    <view class="recharge-panel">
      ...
      <view class="recharge-input" v-if="setting.is_custom == 1">
        <input type="digit" :placeholder="isFocus?'':placeholder" v-model="inputValue" @input="onChangeMoney" @focus="isFocus=true" @blur="isFocus=false" />
      </view>
      <!-- 确认按钮 -->
  ...
  export default {
    data() {
      return {
        ...
        userInfo: {},
        // 充值设置
        setting: {is_custom:1,describe:'充值实时到账，体现2h内到账'},
        // 充值方案列表
        planList: [{plan_id:1,money:100,gift_money:10}],
        // 按钮禁用
        disabled: false,
        // 当前选中的套餐id
        selectedPlanId: 0,
    ...
    computed:{
      ...mapGetters(['memberId','balance','hasLogin'])
    },
    methods: {
      ...
      wxPayment(option) {
        ...
        wxPayment(option)
          ...
          .catch(err => console.log(err,'订单未支付'))
          .finally(() => {
            app.disabled = false;
          })
      }
