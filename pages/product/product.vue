...
<template>
  <view class="container">
    ...
    <view class="introduce-section">
      <text class="title">{{ spu.name }}</text>
    ...
    <view class="share-section" @click="share">
      ...
      <text class="tit">好的东西总要与朋友一起分享！</text>
    ...
    <view class="c-list">
      <view class="c-row b-b" @click="toggleSpec">
        ...
        <view class="con">
          <text
            class="selected-text"
            v-for="(sItem, sIndex) in selectedSpecValueIdArr"
            :key="sIndex"
            >{{ sItem.value }}</text
          >
        </view>
    ...
    >
      ...
      <view class="layer attr-content" @click.stop="stopPrevent">
        ...
        <view v-for="(item, index) in specs" :key="index" class="attr-list">
          <text>{{ item.name }}</text>
          <view class="item-list">
            <text
              v-for="(valueItem, valueIndex) in item.values"
              v-if="valueItem.specId === item.id"
              :key="valueIndex"
              class="tit"
              :class="{ selected: valueItem.selected }"
              @click="selectSpec(index, valueIndex)"
            >
              {{ valueItem.value }}
            </text>
          </view>
...
export default {
  ...
  methods: {
    ...
    selectSpec(specIndex, valueIndex) {
      this.specs[specIndex].values.forEach((item, index) => {
        if (index == valueIndex) {
      ...
      this.selectedSpecValueIdArr = [];
      this.specs.forEach((spec) => {
        const selectedSpecValue = spec.values.filter(
          (value) => value.selected == true
        )[0];
        this.selectedSpecValueIdArr.push(selectedSpecValue);
      });

      const selectedSpecValueIds = this.selectedSpecValueIdArr
        .map((item) => item.id)
        .sort()
        .join(",");

      // 根据规格排序字符串找到匹配的sku信息
      const { id, pic, price } = this.skus.filter(
        (sku) => sku.specValueIds == selectedSpecValueIds
      )[0];
