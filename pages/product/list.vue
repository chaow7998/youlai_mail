...
export default {
  ...
  methods: {
    ...
    loadData(type = "add", loading) {
      ...
      if (type === "add") {
        // 上滑加载
        if (app.loadingType === "nomore") {
          return;
        }
        app.queryParams.page = app.queryParams.page + 1;
        app.loadingType = "loading";
      } else {
        // 刷新
        app.loadingType = "more";
        app.queryParams.page = 0;
      }
      ...
      app.queryParams.categoryId = app.cateId;
      getGoodsList(app.queryParams)
        .then((response) => {
          const { data, total } = response;
          app.goodsList = app.goodsList.concat(data);
          //判断是否还有下一页，有是more  没有是nomore(测试数据判断大于20就没有了)
          app.loadingType = app.goodsList.length >= total ? "nomore" : "more";
          if (type === "refresh") {
            if (loading == 1) {
              uni.hideLoading();
            } else {
		...
		.finally(()=>{
			...
			if (app.filterIndex === 1) {
					  app.goodsList = app.goodsList.sort((a, b) => b.sales - a.sales);
			}
			if (app.filterIndex === 2) {
					  app.goodsList = app.goodsList.sort((a, b) => {
						  if (app.priceOrder == 1) {
