  ...
  <view class="container">
    <view v-if="list.length" class="coupon-list">
      <view class="coupon-item" v-for="(item, index) in list" :key="index">
        <view
          class="item-wrapper"
          :class="['color-' + color[index % color.length]]"
        >
          <view class="coupon-type">{{ item.title }}</view>
          <view class="tip dis-flex flex-dir-column flex-x-center">
            <view>
              <text class="f-30">￥</text>
              <text class="money">{{ item.price }}</text>
            </view>
            <text class="money">5折</text>
            <text class="pay-line">满{{ item.conditionPrice }}元可用</text>
          </view>
    ...
    <empty v-else :isLoading="isLoading" />
  </view>
</template>

<script>
import getCouponList from "@/api/sms/coupon";
