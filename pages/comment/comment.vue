      ...
      >
        ...
        <view class="images-list clearfix" v-if="item.images.length">
          ...
          >
            <image
              ...
              mode="aspectFill"
              :src="image.image_url"
              @click="onPreviewImages(index, imgIdx)"
            ></image>
          </view>
        </view>
        <!-- 商品规格 -->
        <view class="goods-props clearfix">
          <view
            ...
            v-for="(props, idx) in item.orderGoods.goods_props"
            :key="idx"
          >
  ...
  methods: {
    ...
    getCommentList(pageNo = 1) {
      ...
      return new Promise((resolve, reject) => {
        CommentApi.list(
          app.goodsId,
          { scoreType: app.getScoreType(), page: pageNo },
          { load: false }
        ).then((result) => {
          // 合并新数据
    ...
    getTotal() {
      const app = this;
      CommentApi.total(app.goodsId).then((result) => {
        // tab标签内容
