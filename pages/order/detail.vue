<template>
  <view v-if="!isLoading" class="container">
    <view class="header">
      ...
      <view class="order-status">
        <view class="status-icon">
          ...
          <block v-if="order.order_status == OrderStatusEnum.NORMAL.value">
            ...
            <block
              v-else-if="
                order.receipt_status == ReceiptStatusEnum.NOT_RECEIVED.value
              "
            >
              <image
                class="image"
          ...
          <block
            v-if="
              ...
              order.order_status == OrderStatusEnum.APPLY_CANCEL.value
            "
          >
            <image
              class="image"
      ...
      <view
        class="next-action"
        v-if="order.order_status == OrderStatusEnum.NORMAL.value"
      >
        <view
    ...
    <view class="delivery-address i-card">
      ...
      <view class="address">
        <text
          class="region"
          v-for="(region, idx) in order.address.region"
          :key="idx"
          >{{ region }}</text
        >
    ...
    <view
      v-if="
        order.delivery_type == DeliveryTypeEnum.EXPRESS.value &&
        order.delivery_status == DeliveryStatusEnum.DELIVERED.value
      "
      class="express i-card"
      @click="handleTargetExpress()"
    >
      <view class="main">
      ...
      <view class="goods-item" v-for="(goods, idx) in order.goods" :key="idx">
        <view class="goods-main" @click="handleTargetGoods(goods.goods_id)">
          ...
          <view class="goods-content">
            <view class="goods-title twolist-hidden"
              ><text>{{ goods.goods_name }}</text></view
            >
        ...
        <view class="goods-refund">
          ...
          <view
            ...
            class="action-btn"
            @click.stop="handleApplyRefund(goods.order_goods_id)"
            >申请售后</view
          >
        </view>
      </view>
    ...
    >
      <view class="pay-popup">
        ...
        <view class="pop-content">
          ...
          <view
            class="pay-item dis-flex flex-x-between"
            @click="onSelectPayType(PayTypeEnum.BALANCE.value)"
          >
            <view class="item-left dis-flex flex-y-center">
...
export default {
  data() {
    return {
      ...
      order: {
        ...
        user_id: 10002,
        is_delete: 0,
        create_time: "2021-07-09 11:41:00",
        isAllowRefund: false,
        goods: [
          {
            order_goods_id: 3,
            ...
            is_user_grade: 1,
            grade_ratio: 1,
            grade_goods_price: "8710.24",
            grade_total_money: "177.76",
            coupon_money: "0.00",
            points_money: "0.00",
            points_num: 0,
            points_bonus: 8710,
            total_num: 1,
            total_price: "8710.24",
            total_pay_price: "8710.24",
            is_comment: 0,
            order_id: 3,
            user_id: 10002,
            goods_source_id: 0,
            goods_image:
              "http://www.yoshop1.com/uploads/10001/20210621/f165b23e6fc4c57879e3589a341fa1d6.jpg",
            goods: {
              goods_id: 10005,
              goods_name: "美女",
              goods_no: "1111",
              ...
              spec_type: 20,
              goods_price_min: "133.00",
              goods_price_max: "465.00",
              line_price_min: "444.00",
              line_price_max: "444.00",
              stock_total: 1153,
              content:
                '<p><img src="http://www.yoshop1.com/uploads/10001/20210709/c12eac9c738de59b03c14c15137719d5.jpg"/></p><p><img src="http://www.yoshop1.com/uploads/10001/20210709/c0efa20fedbf1b7ef291d1fa42e5a9e2.jpg"/></p><p><img src="http://www.yoshop1.com/uploads/10001/20210621/f165b23e6fc4c57879e3589a341fa1d6.jpg"/><br/></p><p><br/></p>',
              delivery_id: 10001,
              is_points_gift: 1,
  ...
  methods: {
    ...
    //   this.$navTo("pages/product/product", { goodsId });
    // },
    handleTargetGoods(id){
      uni.navigateTo({ url: "/pages/product/product?id=64" })
    },
    ...
    onReceipt() {
      ...
      uni.showModal({
        ...
        success(o) {
          if (o.confirm) {
            OrderApi.receipt(app.orderId).then((result) => {
              // 显示成功信息
              app.$success(result.message);
              // 刷新当前订单数据
              app.getOrderDetail();
