        ...
        >
          ...
          >
            <view class="i-top b-b">
              ...
              <text class="status" style="#fa436a">{{
            ...
            <scroll-view
              ...
              class="goods-box"
              scroll-x
              @click="navTo('/pages/order/detail', order.id)"
            >
              <view
            ...
            >
              ...
              <view class="right">
                ...
                <text class="attr-box"
                  >{{ orderItem.skuName }} x {{ orderItem.skuQuantity }}</text
                >
                <text class="price">{{
            ...
            <view class="price-box">
              共<text class="num">{{ order.totalQuantity }}</text
              >件商品 实付款
              <text class="price">{{ order.payAmount | moneyFormatter }}</text>
            </view>
            <view class="action-box b-t" v-if="order.status === 101">
              <button class="action-btn" @click="cancelOrder(order)">
                取消订单
              </button>
              <button class="action-btn recom" @click="doPay(order)">
                立即支付
              </button>
            </view>
          </view>
...
import { cancelOrder, deleteOrder, orderList } from "@/api/oms/order.js";
...
export default {
  ...
  onLoad(options) {
    ...
    this.tabCurrentIndex = this.navList
      .map((item) => item.status)
      .indexOf(parseInt(options.status));
    this.loadData();
  },
  ...
  methods: {
    loadData() {
      ...
      setTimeout(() => {
        ...
        orderList(params).then((response) => { 
          ...
          const orderAllList = response.data;
		  if(status != 0){
			  const orderList = this.orderFilter(orderAllList,status ) ;
			  navItem.orderList = orderList;
			}else{
				navItem.orderList = orderAllList;
		  }
    ...
    // 筛选当前订单状态标签数据
    orderFilter(orderAllList, status) {
      return orderAllList.filter((orderAllList) => {
        return orderAllList.status == status
      });
    },

    // 跳转到订单详情
    navTo(url, orderId) {
      uni.navigateTo({ url: `${url}?orderId=${orderId}` });
    },
  },
};
</script>
